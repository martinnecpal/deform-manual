---
lang: sk
title: "49.1. 2D Die Stress Study"
---

# 49.1. 2D Die Stress Study

49.1.1. How to add Die stress study for 2D setup

49.1.2. Step selection

49.1.3. Objects window
...
49.1.5.3. Object Mesh

49.1.5.4. Object Material

49.1.5.5. Object boundary conditions

49.1.5.6. Initialize

49.1.5.7. Force Interpolation
...
49.1.8. Simulation controls

49.1.9. Generate DB

## How to add Die stress study for 2D setup

After the completion of a 2D Forming operation, Die stress study can be added in MO wizard from the menu listed after clicking the ![]({{ '/assets/icons/pre_icons/mo_add_operation_icon.jpg' | relative_url }}) button in top left corner (See Fig. 49.1.1. and Fig. 49.1.2.), after adding the die stress study, a new tab is added and die stress operation will be automatically added to the operation editor in the new tab. User can also add multiple die stress study tabs to a single MO project at different steps. MO project at different steps.
...
User can add required number of additional objects for the simulation by clicking ![]({{ '/assets/icons/pre_icons/mo_add_icon2.jpg' | relative_url }}) button. Fig. 49.1.5. shows the objects passed from nominal operation to die stress operation. User can add necessary fixtures and other components based on the setup.
...
In this window user can set temperature for the object and select type of the object as shown in Fig. 49.1.6. For dies passed on from nominal setup, object type is selected as Elastic by default. If the user does not want to calculate stresses on the object, then user can select the object type as Rigid. From v13.1.1, two new options have been introduced in the object page to position the dies in future steps, see Fig. 49.1.6.  
Die positioning in future steps in multi-step die stress study:  
In a multiple step die stress study, if the object requires positioning, then the user needs to choose how the object must be positioned in future steps by turning on the Need positioning check box in object page as shown in Fig. 49.1.6\. 
...
Mesh Page provides options to mesh the object. All the dies on which stresses need to be calculated should be meshed. Mesh page provides the option to user to set number of elements using slider bar or by entering manually to generate mesh, see Fig. 49.1.8. Users can use expert mode to define the desired mesh, please refer [13.1. 2D Mesh Generation](/docs/sk/pre_processor/13_mesh_generation/13_1_2d_mesh_generation/) for more information related to expert mesh options. 
...
When we interpolate the forces from billet, forces are applied to dies, this makes the dies to displace erratically during simulation. To prevent erratic displacement of dies, suitable boundary conditions should be assigned. Velocity BCC can be used to arrest the displacement of dies, see Fig. 49.1.10. Velocity BCC is assigned to top edge of the die is shown in the Fig. 49.1.11.
...
Shrink Fit BCC can be used to define shrink fit conditions between a die insert and a shrink ring. For 2D objects, now we can define shrink fit using Auto and User options.

  * **Auto** – System will automatically determine the tolerance between the selected edge of the current object and its nearest object.

  * **User** – User need to enter the interference value manually by,

    * Selecting the direction, select direction perpendicular to the inner surface of the shrink ring or outer surface of the Die insert.

    * Selecting the inner surface of the shrink ring or outer surface of the Die insert, surface which is contact with the die.

    * Defining shrink fit value - If shrink fit is applied to the inner object, the value should be negative and if shrink fit is applied to the outer object then the value should be positive.

Fig. 49.1.12. & Fig. 49.1.13. shows shrink fit BCC applied to shrink ring. For more information on shrink fit, Please refer [2D Die Stress Analysis - Theory.]()
...
To interpolate the forces from workpiece object to the dies, user need to select ![]({{ '/assets/icons/pre_icons/mo_interpolate_force_button.jpg' | relative_url }}) in Force interpolation window, user can also delete the Interpolated force by using ![]({{ '/assets/icons/pre_icons/mo_delete_interpolate_force_button.jpg' | relative_url }}) option (See Fig. 49.1.15. and Fig. 49.1.16.).

![]({{ '/assets/images/operation_templates/49_die_stress_study/49_1_2d_die_stress_study/image0014.jpg' | relative_url }})

Force Interpolate window
...
The purpose of inter-object relations is to define how the different objects in a simulation interact with each other. In Inter-object window, all the possible relations are already present in the list (See Fig. 49.1.19.). User needs to define the type of relation and Friction value for objects that are in contact for the selected relation.

![]({{ '/assets/images/operation_templates/49_die_stress_study/49_1_2d_die_stress_study/image0018.jpg' | relative_url }})

Inter Object window (Guided mode)
...
**Step increment control:** Solution step size is controlled by time step. Maximum elapsed process time per step can be defined as 1 sec for Die stress analysis operations. Fig. 49.1.20. shows the Simulation Controls window.
...
**Generate database:** By clicking on ![]({{ '/assets/icons/pre_icons/mo_generate_database.jpg' | relative_url }}) button, it generates the Database for the setup. (See Fig. 49.1.21.)
...
[49\. Introduction to Die Stress Study](/docs/sk/operation_templates/49_die_stress_study/49_introduction_to_die_stress_study/)
