...
# 48.1. Spinning Manual

48.1.1. How to add the Spinning Operation

48.1.2. Process

48.1.3. Simulation setup

48.1.4. Objects selection for setup

48.1.5. Workpiece page

48.1.5.1. Defining Workpiece 2D cross-section

48.1.5.2. Workpiece Mesh page

48.1.5.3. Material Page

48.1.5.4. Defining Workpiece BCC

48.1.6. Defining Mandrel Object

48.1.6.1. Defining Mandrel 2D Cross-section

48.1.6.2. Generating Mandrel 3D Geometry

48.1.6.3. Generating Mandrel Mesh

48.1.6.4. Defining Mandrel Material

48.1.6.5. Generating Mandrel BCC

48.1.6.6. Reference point setup for Mandrel

48.1.7. Defining Roll Object

48.1.7.1. Defining Roll Geometry, Mesh, Material and BCC

48.1.7.2. Roll Orientation Page

48.1.8. Pass table

48.1.9. Controls

48.1.9.1. Automatic Positioning

48.1.9.2. Advanced Object Positioning

48.1.10. Contact

48.1.11. Stopping controls

48.1.12. Simulation controls

48.1.13. Generate DB

## How to add the Spinning Operation

Spinning operation can be setup in Integrated Manufacturing Process environment that can be accessed from GUI Main. Create a new problem by either selecting File ![]({{ '/assets/icons/pre_icons/arrow_front.jpg' | relative_url }}) New Problem or by clicking the New Problem ![]({{ '/assets/icons/pre_icons/mo_new_problem_icon.jpg' | relative_url }}) icon. Select 3D Spinning radio button under problem type and unit system. Click on ![]({{ '/assets/icons/pre_icons/mo_ok_button.jpg' | relative_url }}) button (as shown in Fig. 48.1.1.). Integrated Manufacturing Process template will open, we can see that 3D Spinning operation is added in Operation editor.
...
We can also add Spinning operation into Integrated Manufacturing Process environment from the New Project pop-up when a new problem is opened in Integrated Manufacturing Process environment as shown in Fig. 48.1.2. Using “Copy Existing project” option, we can import previous saved projects as new project from the New Project pop-up.
...
We can also add Spinning operation to operation editor from explorer tab in Integrated Manufacturing Process environment, by clicking on ![]({{ '/assets/icons/pre_icons/mo_add_operation_icon.jpg' | relative_url }}) button next to Spinning operation or by drag and drop Spinning operation into operation editor window. As the Spinning operation is added into operation editor, If the current Screen upward direction is "Z" direction then we will get below popup as shown in Fig. 48.1.3. Click ![]({{ '/assets/icons/pre_icons/mo_yes_change_button.jpg' | relative_url }}) in "Change screen upward axis" popup.
...
Now the Process selection page will be opened in property settings modification window as shown in Fig. 48.1.4.
...
In the process page by default “Spinning” option is selected. We can use spinning operation template to set up “Flow forming” operation. We can define the shaft rotational speed (w) in process page as shown in the Fig. 48.1.5.
...
Spinning setup behavior based on solution and solver selection is explained in below table,
...
The user can select the objects to be used in the setup from the available object types as shown in the Fig. 48.1.7. User can select the “Number of rolls” as used in the setup. If the rolls are more than 1 then angle between them in counter-clock direction can be define in the table below “Number of rolls” field as shown in Fig. 48.1.7.
...
The user can define the Workpiece name, temperature and Object type as shown in the Fig. 48.1.8. Depending on the solver selection in Simulation setup user need to select the object type. Plastic object type for Implicit solver and Elasto-plastic object type for Explicit solver. User can import objects from other databases or key files using ![]({{ '/assets/icons/pre_icons/mo_import_file_icon.jpg' | relative_url }}) and ![]({{ '/assets/icons/pre_icons/mo_load_from_library_icon.jpg' | relative_url }}) options or save the object data using ![]({{ '/assets/icons/pre_icons/mo_save_to_a_file_icon.jpg' | relative_url }}) and ![]({{ '/assets/icons/pre_icons/mo_save_to_library_icon.jpg' | relative_url }}) options.
...
The user can define the 2D Cross section of the workpiece using the ![]({{ '/assets/icons/pre_icons/mo_define_primitive_label.jpg' | relative_url }}) or ![]({{ '/assets/icons/pre_icons/mo_edit_lable.jpg' | relative_url }}) option. Center and Axis of the geometry is defined in “2D cross-section info” as shown in the Fig. 48.1.9. The system assumes that the geometry will be revolved about the X-axis by default. Therefore, the center axis of the 2D cross-section geometry should be colinear with the global X-axis. User can import 2D Cross-section from other databases or key files using ![]({{ '/assets/icons/pre_icons/mo_import_file_icon.jpg' | relative_url }}) , ![]({{ '/assets/icons/pre_icons/mo_load_from_library_icon.jpg' | relative_url }}) options or save the 2D Cross-section data using ![]({{ '/assets/icons/pre_icons/mo_save_to_a_file_icon.jpg' | relative_url }}), ![]({{ '/assets/icons/pre_icons/mo_save_to_library_icon.jpg' | relative_url }}) options.
...
Spinning setup uses brick mesh as initial mesh generation method for both Lagrangian and ALE. If brick mesh generation has any issues in Lagrangian type of simulation, then tetrahedral mesh will be used during remeshing. User can generate the cross-section mesh for defined settings by clicking on ![]({{ '/assets/icons/pre_icons/mo_generate_2d_mesh_button.jpg' | relative_url }}) label and 3D mesh with defined settings can be generated by clicking on ![]({{ '/assets/icons/pre_icons/mo_generate_3d_mesh_button.jpg' | relative_url }}), see Fig. 48.1.10.

#### Cross-section mesh

  * **Number of Elements:** User can define number of elements to be used in cross-section mesh here, see Fig. 48.1.10. System generated Cross-section mesh when user clicks on ![]({{ '/assets/icons/pre_icons/mo_generate_2d_mesh_button.jpg' | relative_url }}) label which will be rotated to generate 3D Mesh.

####   
3D meshing parameters

  * **Number of Revolved sections:** The user can generate the 3D mesh by defining the number of revolved sections along the hoop direction and click on ![]({{ '/assets/icons/pre_icons/mo_generate_3d_mesh_button.jpg' | relative_url }}), see Fig. 48.1.10.

  * **Uniform thickness of layers in hoop direction:** When this option is used then 3D mesh is generated with layers of uniform thickness along the hoop direction, see Fig. 48.1.10. This option is preferred for “Explicit” type of simulation setup.

  * **Finer mesh in contact region:** User can generate finer mesh at contact region by specifying the angle in the “Angle” field. A fine mesh with the size ratio specified in “Size ratio” field will be generated within specified angle at contact with rolls, see Fig. 48.1.10.
...
Mandrel is a very important object in Spinning as the material flows along the Mandrel surface taking the shape of the mandrel. Mandrel can be simple cylinder or may have some profile. Mandrel object type is “Rigid”. In Mandrel object page user can define the Object name and temperature (see Fig. 48.1.13.). The user can import object from other databases or key files using ![]({{ '/assets/icons/pre_icons/mo_import_file_icon.jpg' | relative_url }}) , ![]({{ '/assets/icons/pre_icons/mo_load_from_library_icon.jpg' | relative_url }}) options or save the object data using ![]({{ '/assets/icons/pre_icons/mo_save_to_a_file_icon.jpg' | relative_url }}), ![]({{ '/assets/icons/pre_icons/mo_save_to_library_icon.jpg' | relative_url }}) options.
...
2D cross-section for Mandrel can be defined similar to the workpiece 2D Cross-section as explained in Workpiece Defining Workpiece 2D cross-section. User can define the 2D Cross-section which will be used to generate 3D Geometry.

###  Generating Mandrel 3D Geometry 

User need to generate 3D geometry for all objects in Spinning operation setup. Flow forming/ Spinning operation uses 2D Cross-section to generate 3D by revolving about defined axis and center at Workpiece object page. After defining the digitalization options, Number of revolved sections and Finer geometry in contact region user need to click on ![]({{ '/assets/icons/pre_icons/mo_generate_3d_geometry_button.jpg' | relative_url }}) button to create a 3D geometry as shown in the Fig. 48.1.14.

**Digitization:** “Length tolerance fraction”/ “Maximum allowable angle”/ “Minimum allowable angle” can be used to control digitization points on the 2D cross-section for accurate representation of 2D Cross-section, see Fig. 48.1.14.
...
If user wants to perform thermal calculations in a Non-isothermal simulation setup then user can generate mesh for mandrel. Mesh settings and Mesh generating procedure is similar to Generating Workpiece Mesh.

### Defining Mandrel Material 

User can assign the material from the list on load from Library similar to Defining Workpiece Material.
...
User can define reference point for mandrel and position the mandrel with respect to DEFORM origin along X-axis. The system will display calculated value of D from DEFORM origin using the present position of mandrel as shown in Fig. 48.1.15. “D” is the distance between the Deform Origin and the origin of the Mandrel along X axis and will be used to position the mandrel.
...
Each Roll must be defined separately. Rolls are of Rigid Object types and can be meshed if thermal calculations need to be performed in non-isothermal simulation setup. In roll’s object page user can define the Object name and temperature. User can import roll’s object from other databases or key files using ![]({{ '/assets/icons/pre_icons/mo_import_file_icon.jpg' | relative_url }}), ![]({{ '/assets/icons/pre_icons/mo_load_from_library_icon.jpg' | relative_url }}) options or save the object data using ![]({{ '/assets/icons/pre_icons/mo_save_to_a_file_icon.jpg' | relative_url }}), ![]({{ '/assets/icons/pre_icons/mo_save_to_library_icon.jpg' | relative_url }}) options.

### Defining Roll Geometry, Mesh, Material and BCC

Roll geometry, Mesh (in case of non-isothermal setup), Material and BCC can be defined similar to methodology as explained in 48.1.5.1. Defining Workpiece 2D cross-section, 48.1.6.2. Generating Mandrel 3D Geometry, 48.1.5.3. Defining Workpiece Material and 48.1.5.4. Defining Workpiece BCC.

### Roll Orientation Page

In spinning operation rolls require a unique setup approach due to their use of the path movement control. Roll translation will be defined by a path that is applied in a local UV (axial/radial) coordinate system. A single point on the roll, the reference point (datum), will follow this path. The current location of the reference point, and thus the roll, is determined by the path data and the current time. Each roll following a path must have a reference point defined for it.  
User can select the Roll datum point using the button ![]({{ '/assets/icons/pre_icons/mo_tolerance_icon.jpg' | relative_url }}) next to Roll Datum field (See Fig. 48.1.16.) which will provide options of “Rotation center”, “Nose center” and “Corner point” as shown in Fig. 48.1.16. to be used as “Roll Datum point”, based on the selection the datum point co-ordinates are automatically calculated. When picking the roll datum point, the user should select a point on the black half of the displayed roll cross-section. User can also enter the datum co-ordinates manually. User can define the “Orientation Angle” with respect to X-axis as shown in Fig. 48.1.17. The orientation angle must be applied before the roll datum point is defined. The roll datum point must be reselected whenever the orientation angle is modified.

![]({{ '/assets/images/operation_templates/48_spinning/48_1_spinning_manual/image0016.jpg' | relative_url }})

Roll Orientation Page - Datum Wizard
...
Spinning process may involve single pass or multiple passes, user can define the multi pass and multi roll movement data using “Pass Table” as shown in the Fig. 48.1.18. When we click on the button we will get the Roll movement controls as shown in the Fig. 48.1.19. The user can define the Translation movement using the “[Speed](/docs/sk/pre_processor/15_movement_controls_definition/15_1_speed/)” and “[Path](/docs/sk/pre_processor/15_movement_controls_definition/15_8_path/)” type. For the Rotation movement we have Angular velocity and Torque type. 

“Path” type translation movement is commonly used in Spinning process. For more information on how to define “[Path](/docs/sk/pre_processor/15_movement_controls_definition/15_8_path/)” type movement please refer [15\. Movement controls](/docs/sk/pre_processor/15_movement_controls_definition/15_movement_controls_settings/). After defining the path movement data user can use the ![]({{ '/assets/icons/pre_icons/mo_show_path_movement_info_in_2d_button.jpg' | relative_url }}) to display the path that roller datum is going to trace in 2D as shown in the Fig. 48.1.20.
...
Objects positioning can be varied using ![]({{ '/assets/icons/pre_icons/mo_automatic_positioning_button.jpg' | relative_url }}) and “Advanced Object Positioning" ![]({{ '/assets/icons/pre_icons/mo_positioning_objects_button.jpg' | relative_url }}) options available in “Controls” page as shown in Fig. 48.1.21.
...
In Spinning the automatic positioning is mostly used for path base roller positioning. We have two positioning options they are “Initial (t=0)” and “At contact” as shown in the Fig. 48.1.22.  
The **Initial (t=0)** is used to position the workpiece along defined path coordinates when time is zero, see Fig. 48.1.23.  
The **At contact** option is used to move the roll in the define path until it makes the contact with workpiece and updates the time, see Fig. 48.1.24.
...
The user can define the process duration as the stopping criteria to stop the simulation as shown in the Fig. 48.1.28.
...
The user can define the Step controls for simulation as shown in the Fig. 48.1.29.
...
**Check Data![]({{ '/assets/icons/pre_icons/mo_check_data_button.jpg' | relative_url }}) : **When user clicks on this label system checks the Data. If Data is correct, then we can generate DB. But while checking Data if it gives any errors or warnings then it should be corrected before generating Database. Errors will not allow the database to be generated while warnings will allow the DB to be generated.

**Generate Database** ![]({{ '/assets/icons/pre_icons/mo_generate_database.jpg' | relative_url }}): User can generate database by clicking on this label. (See Fig. 48.1.30.)  
