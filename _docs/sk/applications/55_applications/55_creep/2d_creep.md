...
To run a creep simulation, the following requirements must be fulfilled:
  ...
  * Creep (CREEP) has to be activated in the **Object Properties![]({{ '/assets/icons/pre_icons/arrow_front.jpg' | relative_url }}) **[**Deformation**](/docs/sk/pre_processor/16_object_properties/16_1_deformation_properties/) tab

  * A creep model with non-trivial data has been defined (any of the given model is sufficient). In the case of [user routines](/docs/sk/user_routines/user_routine_mainpg/), the required routine should have the routine compiled and the simulation should run the special FEM engine.
