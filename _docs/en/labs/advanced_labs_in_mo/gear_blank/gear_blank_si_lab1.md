...
Select the **Material List** in operation tree to load the material from library.
...
Before we proceed further, go to **Options** → **Preferences** menu option and ensure that **Y- axis is up** is the preferred upward axis. Click ![]({{ '/assets/icons/pre_icons/mo_ok_button.jpg' | relative_url }}) button.
...
Click on **Movement** under Top die in operation tree as this is an isothermal simulation there is no need to generate mesh or assign material to top die.
...
Keep the Bottom die default temperature **20** °C and confirm its object type is **rigid**. Click on ![]({{ '/assets/icons/pre_icons/mo_next_button.jpg' | relative_url }}) button to define the bottom die geometry.
...
Click ![]({{ '/assets/icons/pre_icons/mo_ok_button2.jpg' | relative_url }}) to close the geometry definition and click on **Positioning** in the operation tree to open the objects positioning window.
...
![]({{ '/assets/images/labs/advanced_labs_in_mo/gear_blank/gear_blank_si/image0029.jpg' | relative_url }})
...
The graphics region will display Simulation graphics. In Simulation graphics you can observe the progress of a simulation as it is running, by default it updates the last saved step. To update the current step user has to select pulldown button next to Update button and select "Current step".
...
When the simulation is completed, click the ![]({{ '/assets/icons/pre_icons/mo_simulation_mode_button.jpg' | relative_url }}) Mode button to review results.
...
![]({{ '/assets/images/labs/advanced_labs_in_mo/gear_blank/gear_blank_si/image0031.jpg' | relative_url }})
...
Select step **75**(or any other intermediate step on the step selection bar). Click the ![]({{ '/assets/icons/post_icons/mo_vel_sv_icon.jpg' | relative_url }}) (Total Velocity) state variable icon in the Post tools State Variables short cuts. Click the ![]({{ '/assets/icons/post_icons/mo_state_variable_icon.jpg' | relative_url }}) icon to open the state variable properties dialog. Select the **Vector****Plot** display type and click ![]({{ '/assets/icons/pre_icons/mo_apply_button.jpg' | relative_url }}) button to apply the changes. The instantaneous velocities are displayed as vectors at the selected time step. Colors are included to highlight the magnitude of the velocity. (See Fig. L1.31.)

  * **Effective strain plot**

Select **Strain****Effective****** from state variable dialog ![]({{ '/assets/icons/post_icons/mo_deformation_icon.jpg' | relative_url }}) (Deformation) variables list. This represents the amount of deformation during the simulated process. In cold forming, this is related to work hardening.
