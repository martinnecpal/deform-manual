...
User can define (the maximum allowable) time step size as a function of “stroke” ([DSMAX](/docs/en/keyword_documentation/d/dsmax/)) or “time” ([DTMAX](/docs/en/keyword_documentation/d/dtmax/)).  
...
There are three modes ([STPDEF](/docs/en/keyword_documentation/s/stpdef/)) for defining steps,

  * **User** In user defined steps mode, the steps correspond to the [NSTEP](/docs/en/keyword_documentation/n/nstep/) value. This is the default which does not have to be changed in almost all cases.
  ...
  * **Temperature** In temperature based sub stepping, the [DTPMAX](/docs/en/keyword_documentation/d/dtpmax/) settings control the time stepping. The purpose for these controls is to specify the time stepping of a simulation that is driven by thermal-induced deformation.
...
The maximum element strain increment limits the amount of strain that can accumulate in any individual element during one time step. If a non-zero value is assigned to [DEMAX](/docs/en/keyword_documentation/d/demax/), a new sub step will be initiated when the strain increment in any element reaches the value of [DEMAX](/docs/en/keyword_documentation/d/demax/).

  * **Contact Time (DTSUB) [2D, 3D]**

After either DSMAX or DTMAX is prescribed, the deformation simulation may be broken into smaller sub steps, depending on the assigned value of [DTSUB](/docs/en/keyword_documentation/d/dtsub/).  
  
**[2D]** : For a default [DTSUB](/docs/en/keyword_documentation/d/dtsub/)= 0.0, the system will enable contact sub-stepping.  
For [DTSUB](/docs/en/keyword_documentation/d/dtsub/)= 1.0, no sub-stepping will be done.  
  
**[3D]** : Contact time controls whether or not sub stepping is performed when nodes contact a master surface. By default ([DTSUB](/docs/en/keyword_documentation/d/dtsub/)= 1), if a node contacts a master surface a fraction of the way through a time step, the time step is subdivided, and that step is run again at the fraction of the time increment. This will place the node on the surface at the end of the time step. For 3D problems with a large number of nodes contacting master surfaces, this can cause huge increases in execution time.  
  
If [DTSUB](/docs/en/keyword_documentation/d/dtsub/)is set to 1, contact time sub stepping is disabled. Nodes will be allowed to penetrate the master surface, but then will be artificially moved back to surface at the end of the time step. This will allow significantly faster execution time. However, if the defined time step is too large, some volume loss and mesh distortion may occur.  
In general, it is recommended that [DTSUB](/docs/en/keyword_documentation/d/dtsub/)be set to 1 and that the time step guidelines described above be followed carefully. Use of polygon length sub stepping, [DPLEN](/docs/en/keyword_documentation/d/dplen/), will also control volume loss and mesh distortion, without severe execution time increase.

  * **Polygon length sub step (DPLEN) [3D]**

Polygon length sub stepping ([DPLEN](/docs/en/keyword_documentation/d/dplen/))places an upper limit on the absolute distance a surface node can move in a given time step.  

![]({{ '/assets/equations/pre_processor/9_simulation_controls/9_2_defining_step/eq_9_2_2.jpg' | relative_url }}) |   
---|---  
  
  
Legal values of [DPLEN](/docs/en/keyword_documentation/d/dplen/) are from 0 to 1. A value of 0 will disable sub stepping. Recommended values are 0.2 to 0.5, with 0.2 being more conservative, and hence slower, and 0.49 being more aggressive, and faster, but less accurate. Values larger than 0.5 can be used, but may allow unacceptable mesh degeneration.
